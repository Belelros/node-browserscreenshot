...
function APIClient(settings, cb){
  ...
  self.getBrowsers((err, json) => {
    if (err){ throw err; }
    self.storeBrowsers(...[json,cb]);
  });
}
/**
 ...
 * @returns {String}
 */
APIClient.prototype.getUserAgent = () => [[os.platform(),os.release()].join('/'), ['node',process.versions.node].join('/'), 'node-browserstack'].join(' ');
/**
...
APIClient.prototype.request = function(options, data, cb){
  ...
  var request = http.request(this.getRequestOptions(options,dataString), res => {
    var response = '';
...
APIClient.prototype.storeBrowsers = function(browsers, cb){
  ...
  browsers.forEach(browser => {
    ...
    var osName = browser.os.toLowerCase();
  ...
  this.request({
    ...
    if (data){
      ...
      data.screenshots.forEach((screenshot, index) => {
        functions.push(done => {
          self.queryQueue(...[data.wait_time, data.job_id, index,done]);
        });
      });

      console.log('There are %d images in queue', data.screenshots.length);

      async.parallel(functions,data => {
  ...
  setTimeout(() => {
    self.request({
      ...
      if (data.screenshots[0].state === 'processing'){
        self.queryQueue(...[data.wait_time,jobID, index,cb]);
      }
      else {
        if (data.screenshots[index].state === 'done'){
          console.log('Fetching image %d', index + 1);
          self.fetchImages(...[data.screenshots[index],cb]);
        }
        else {
          self.queryQueue(...[data.wait_time,jobID, index,cb]);
        }
 ...
 */
APIClient.prototype.getImageName = imageData => {
  var os = [imageData.os,imageData.os_version].join('_');
  var browser = [imageData.browser,imageData.browser_version].join('_');
  var domain = imageData.url.substr(imageData.url.indexOf('://')+3).split('.');
...
APIClient.prototype.guessBrowsers = function(browsers, os, osVersions, devices){
  ...
  browsers.forEach(browser => {
    ...
    var browserName = aux.shift();
    var browserVersion = aux.pop();
    ...
    if (self.browsers[browserName]){
      ...
      if (browserStackBrowser.os.length === 1 || !os || arrayInArray(os,browserStackBrowser.os)){
        for (var i = 0, len = browserStackBrowser.list.length; i < len; i++){
          ...
          var currentBrowserOSName = browserStackBrowser.list[i].os.toLowerCase();
          var currentBrowserOSVersion = browserStackBrowser.list[i].os_version.toLowerCase();
          var currentBrowserDevice = browserStackBrowser.list[i].device ? browserStackBrowser.list[i].device.toLowerCase() : null;
...
APIClient.prototype.prepareOS = os => {
  var auxOs = [];
  var osVersions = null;
 ...
 * @returns {boolean}
 */
APIClient.prototype.validBrowserVersion = (currentBrowserVersion, browserVersion) => currentBrowserVersion === browserVersion || browserVersion === 'all' || browserVersion === '0.0' || !currentBrowserVersion;
/**
 ...
 * @returns {boolean}
 */
APIClient.prototype.validOS = (browserStackBrowser, currentBrowserOSName, os) => browserStackBrowser.os.length === 1 || !os  || os.indexOf(currentBrowserOSName) > -1;
/**
 ...
 * @returns {boolean}
 */
APIClient.prototype.validOSVersion = (
  osVersions,
  currentBrowserOSName,
  currentBrowserOSVersion,
  browserStackBrowser
) => !osVersions ||
 ...
 * @returns {boolean}
 */
APIClient.prototype.validDevice = (currentBrowserDevice, devices) => !currentBrowserDevice || !devices || devices.indexOf(currentBrowserDevice) > -1;
